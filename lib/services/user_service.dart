...
class UserService extends ChangeNotifier with WidgetsBindingObserver { // <--- THÊM OBSERVER
  final UserRequest _userRequest = UserRequest();
  final FirebaseAuth _auth = FirebaseAuth.instance;
  ...
  // ✅ HÀM CẬP NHẬT TRẠNG THÁI TRÊN FIRESTORE
  Future<void> updateOnlineStatus(bool status) async {
    ...
    try {
      ...
      _currentUser = _currentUser!.copyWith(isOnline: status);
      notifyListeners();
    } catch (e) {
      print('❌ [UserService] Update status error: $e');
    }
  }

  // ✅ XỬ LÝ LIFECYCLE (ONLINE/OFFLINE KHI TẮT APP)
  @override
  ...
  Future<void> _loadUserWithRetry(String uid, {int retryCount = 0}) async {
    ...
    try {
      ...
      if (_currentUser != null) {
        _cancelRetryTimer();
        updateOnlineStatus(true); // <--- ONLINE KHI VỪA VÀO APP
  ...
  Future<void> _onAuthStateChanged(User? firebaseUser) async {
    ...
    if (_currentUser?.uid == firebaseUser.uid) return;
    await _loadUserWithRetry(firebaseUser.uid);
  }
  ...
  Future<bool> forceReloadUserWithTimeout({Duration timeout = const Duration(seconds: 5)}) async {
    ...
    notifyListeners();
    try {
      _currentUser = await _userRequest.getUserByUid(firebaseUser.uid).timeout(timeout);
      _isLoading = false;
      notifyListeners();
      if (_currentUser != null) {
        updateOnlineStatus(true);
  ...
  @override
  void dispose() {
    WidgetsBinding.instance.removeObserver(this); // <--- HỦY LIFECYCLE
    _cancelRetryTimer();
    _authSubscription?.cancel();
    super.dispose();
  }
