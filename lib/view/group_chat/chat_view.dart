...
//     if (message.type == 'qr_user') return _buildUserQRBubble(context);
...
//     );
//   }
//   Widget _buildUserQRBubble(BuildContext context) {
//     // 1. Xác định căn lề trái/phải
//     final alignment = isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start;
//     final rowAlignment = isMe ? MainAxisAlignment.end : MainAxisAlignment.start;
    
//     // 2. Lấy avatar người gửi (nếu không phải là mình)
...
//               // Widget hiển thị thẻ QR (đã tạo ở bước trước)
//               UserQRMessageBubble(
//                 qrDataString: message.content, // Truyền chuỗi JSON vào
//                 isMe: isMe,
//               ),
//             ],
//           ),
          
//           // Dòng hiển thị thời gian và trạng thái gửi
//           Padding(
//             padding: EdgeInsets.only(
//               top: 4,
//               left: isMe ? 0 : 52, // Thụt lề nếu là người khác gửi
//               right: isMe ? 8 : 0,
//             ),
//             child: Row(
//               mainAxisSize: MainAxisSize.min,
...
import 'package:mangxahoi/view/widgets/chat/chat_view_content.dart';
      ...
      child: Consumer<ChatViewModel>(
        builder: (context, vm, _) {
          if (vm.isGroup) {
            return GroupStatusStreamWidget(
              groupId: chatId,
              child: ChatViewContent(chatName: chatName),
            );
          }
          return ChatViewContent(chatName: chatName);
        },
      ),
    );
  }
