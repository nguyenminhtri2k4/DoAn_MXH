...
class CreatePostView extends StatelessWidget {
  ...
  const CreatePostView({super.key, required this.currentUser, this.groupId});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => PostViewModel(),
      child: _CreatePostViewContent(currentUser: currentUser, groupId: groupId),
    );
  }
}
...
class _CreatePostViewContent extends StatefulWidget {
  ...
  const _CreatePostViewContent({required this.currentUser, this.groupId});
...
class _CreatePostViewContentState extends State<_CreatePostViewContent> {
  ...
  void _navigateToTagFriends() async {
    ...
    final List<String>? selectedIds = await Navigator.push(
      ...
      MaterialPageRoute(
        builder:
            (context) => TagFriendsView(
              friendIds:
                  widget.currentUser.friends, // Truyền danh sách ID bạn bè
              previouslySelectedIds:
                  viewModel.taggedUserIds, // Truyền danh sách đã chọn
            ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final isPostButtonEnabled =
        viewModel.canPost && !viewModel.isLoading && !viewModel.isCheckingVideo;
          ...
          Padding(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor:
                    isPostButtonEnabled ? AppColors.primary : Colors.grey[400],
  ...
  Widget _buildActionBar(BuildContext context, PostViewModel viewModel) {
    ...
    final bool canPickImage =
        viewModel.mediaFiles.length < 6 && !viewModel.hasVideo;
      ...
      child: Row(
        ...
        children: [
          _buildActionButton(
            ...
            onPressed:
                canPickImage ? () => viewModel.pickImages(context) : () {},
          ),
          _buildActionButton(
            icon: Icons.videocam,
            label: 'Video',
            ...
            onPressed:
                canPickVideo ? () => viewModel.pickVideo(context) : () {},
          ),
          _buildActionButton(
            // <-- THAY ĐỔI
            icon: Icons.person_add,
            label:
                viewModel.taggedUserIds.isEmpty
                    ? 'Gắn thẻ'
