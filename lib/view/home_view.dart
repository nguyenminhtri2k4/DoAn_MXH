...
class _HomeViewContentState extends State<_HomeViewContent> {
  ...
  // SỬA LỖI: Thêm cờ để chỉ tải bài viết một lần
  ...
  void initState() {
    ...
    // SỬA LỖI: Xóa lệnh gọi fetchInitialPosts khỏi initState
    // Lệnh này sẽ được chuyển vào hàm build
  ...
  Widget _buildHomePageBody(BuildContext context) {
    ...
    // 1. Kiểm tra xem user service đã tải xong chưa
    if (userService.isLoading || userService.currentUser == null) {
      // Nếu chưa, hiển thị vòng xoay chờ
      return const Center(child: CircularProgressIndicator());
    }
    
    // 2. SỬA LỖI: Khi user service đã tải xong, chúng ta mới bắt đầu tải bài viết
    // Dùng cờ _isPostsLoading để đảm bảo chỉ gọi 1 lần duy nhất
    if (!homeViewModel.isLoading && homeViewModel.posts.isEmpty && !_isPostsLoading) {
      _isPostsLoading = true; // Đánh dấu là đang tải
      
      // Dùng addPostFrameCallback để tránh lỗi "setState during build"
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Bắt đầu tải bài viết
        context.read<HomeViewModel>().fetchInitialPosts(context).then((_) {
          // Sau khi tải xong, đặt lại cờ (nếu widget vẫn còn tồn tại)
          if (mounted) {
    ...
    // 3. Xử lý hiển thị UI như bình thường
    final currentUserId = userService.currentUser!.id;
    final posts = homeViewModel.posts;

    // Nếu đang tải bài viết (lần đầu), hiển thị vòng xoay
    if (homeViewModel.isLoading && posts.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }
