...
//   void _handleScroll() {
//     // Chỉ áp dụng hiệu ứng cho trang chủ
//     if (_selectedIndex != 0) return;

//     if (_scrollController.position.userScrollDirection == ScrollDirection.reverse && _scrollController.offset > kToolbarHeight) {
//       if (_isVisible) {
//         setState(() {
//           _isVisible = false;
//         });
...
//     // Nếu chọn lại tab home, cuộn lên đầu
...
//       // Luôn hiển thị lại các thanh khi chuyển tab
//       _isVisible = true;
//     });
//   }
...
//     // Biến điều khiển hiển thị (chỉ ẩn ở trang chủ)
//     final showUI = isHomePage ? _isVisible : true;

//     final List<Widget> pages = [
//       // Trang chủ chỉ là nội dung, không có Scaffold
//       (vm.isLoading || vm.currentUserData == null)
...
//               },
//             ),
//       // Các trang khác là các widget hoàn chỉnh (có Scaffold riêng)
//       const FriendsView(),
...
class _HomeViewContentState extends State<_HomeViewContent> {
    ...
    return (vm.isLoading || vm.currentUserData == null)
        ...
        : StreamBuilder<List<PostModel>>(
            ...
            builder: (context, snapshot) {
              ...
              final posts = snapshot.data!;
              // ListView bây giờ có padding trên cùng để không bị AppBar che khuất ban đầu
              return ListView.builder(
                controller: _scrollController,
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> pages = [
      _buildHomePageBody(vm), // Chỉ chứa nội dung trang chủ
      const FriendsView(),
      const NotificationView(),
      const VideoView(),
      const ProfileView(),
    ...
    return Scaffold(
      extendBodyBehindAppBar: isHomePage, // Chỉ cho phép body nằm sau AppBar ở trang chủ
      appBar: isHomePage
          ? PreferredSize(
              preferredSize: const Size.fromHeight(kToolbarHeight),
              child: AnimatedContainer(
