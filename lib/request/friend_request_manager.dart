...
class FriendRequestManager {
  ...
  Future<void> sendRequest(String fromUserId, String toUserId) async {
    ...
    if (status == 'pending_sent')
      throw Exception("Đã gửi lời mời, đang chờ chấp nhận.");
    if (status == 'pending_received')
      throw Exception("Người này đã gửi lời mời cho bạn. Hãy kiểm tra.");
  ...
  Future<String> getFriendshipStatus(String userId1, String userId2) async {
    ...
    final user1Doc =
        await _firestore.collection(_userCollection).doc(userId1).get();
  ...
  Future<void> blockUser(String blockerId, String blockedId) async {
    ...
    final blockerUserRef = _firestore
        .collection(_userCollection)
        .doc(blockerId);
    batch.update(blockerUserRef, {
      'friends': FieldValue.arrayRemove([blockedId]),
    });
    ...
    batch.update(blockedUserRef, {
      'friends': FieldValue.arrayRemove([blockerId]),
    });
  ...
  /// Kiểm tra xem currentUserId có đang chặn targetUserId không
  Future<bool> isUserBlocked(String currentUserId, String targetUserId) async {
    try {
      ...
      print('❌ Lỗi khi kiểm tra trạng thái chặn: $e');
      return false;
    }
  }
}
