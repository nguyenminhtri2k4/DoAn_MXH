...
//   final String _userCollection = 'User';s
...
class FriendRequestManager {
  ...
  Future<void> cancelSentRequest(String requestId) async {
    await _firestore.collection(_friendRequestCollection).doc(requestId).update({'status': 'cancelled'});
  }
  ...
  Future<void> rejectRequest(String requestId) async {
    await _firestore.collection(_friendRequestCollection).doc(requestId).update({'status': 'rejected'});
  }
