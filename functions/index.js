...
// });
/**
 * functions/index.js – PHIÊN BẢN HOÀN CHỈNH NHẤT 2025
 */


// 1. IMPORT CÁC THƯ VIỆN CẦN THIẾT
const { onDocumentCreated } = require("firebase-functions/v2/firestore");
const { onRequest } = require("firebase-functions/v2/https"); // Thêm cái này để làm API gọi từ Web
const admin = require("firebase-admin");
const { FirestoreAdminClient } = require('@google-cloud/firestore'); // Thêm thư viện này để backup

admin.initializeApp();

// Logo app khi chưa có avatar
const DEFAULT_AVATAR_URL = "https://firebasestorage.googleapis.com/v0/b/doanmxh-1b  015e.firebasestorage.app/o/user_avatars%2Flogoapp.png?alt=media&token=2da297eb-c060-47ca-a659-f0ad1b78e358";

// ==================================================================
// 4. CHỨC NĂNG BACKUP DỮ LIỆU (Gọi từ Web)
// ==================================================================
exports.backupFirestore = onRequest({ timeoutSeconds: 540 }, async (req, res) => {
  // Cấu hình CORS để Web có thể gọi được function này
  ...
  // TÊN BUCKET ĐỂ LƯU BACKUP
  // Bạn CẦN tạo một bucket riêng trên Google Cloud Console (ví dụ: doanmxh-backups)
  // Nếu chưa có, hãy đổi thành tên bucket chính của bạn: `gs://${projectId}.appspot.com`
  const bucket = `gs://${projectId}.appspot.com`; 

  try {
    const [response] = await client.exportDocuments({
      name: databaseName,
      outputUriPrefix: bucket + "/firestore_backups", // Sẽ lưu vào folder firestore_backups
      // collectionIds: [] // Để trống là backup tất cả, hoặc điền ['User', 'Post'] nếu muốn cụ thể
    });

    console.log(`Backup started: ${response.name}`);
    res.status(200).json({ 
      message: "Đã bắt đầu quá trình sao lưu Database!", 
      operation: response.name 
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Lỗi khi sao lưu: " + err.message });
  }
});

// ... (GIỮ NGUYÊN CÁC HÀM CŨ CỦA BẠN Ở DƯỚI: sendPushNotification, v.v...)
    ...
    const message = {
      ...
      android: {
        notification: {
          channelId: "high_importance_channel",
          priority: "high",
          imageUrl: image && image.trim() !== "" ? image : null,
        },
      },
      apns: { payload: { aps: { sound: "default", badge: 1 } } },
    };
  ...
  try {
    ...
    const payload = {
      ...
      android: {
        ...
        notification: {
          channelId: "high_importance_channel",
          imageUrl: senderAvatar && senderAvatar.trim() !== "" ? senderAvatar : null,
        },
      },
      apns: {
        payload: { aps: { sound: "default", badge: 1, "content-available": 1 } },
...
// ==================================================================
// 3. THÔNG BÁO BÌNH LUẬN & REACTION
// ==================================================================
  ...
  // ... (Code cũ của bạn giữ nguyên)
    ...
    const payload = {
      ...
      android: {
        ...
        notification: {
          channelId: "high_importance_channel",
          imageUrl: actorAvatar && actorAvatar.trim() !== "" ? actorAvatar : null, 
        },
      },
      apns: { payload: { aps: { sound: "default", badge: 1 } } },
    };
